...
function chatHttpHandler(request) {
    if (request.path === "/chat/messages/") { // handle messaging logic
        var bufferedBody = "";
        ...
        request.onEnd = function () {
            request.onWriteReady = function () { request.writeBody(chatui.html, true); };
            request.writeHeaders(200, "OK", { "Content-type": "text/html", "Content-length": chatui.html.length.toString(), "Cache-Control": "no-cache" }, false);
        }
    }
    request.readMore();
}

http.createServer({ url: "http://*:80/chat", onNewRequest: chatHttpHandler }).start(); // start message and UI server
...
tracing.write("Chat server running. Navigate to http://localhost/chat with the browser. Press Ctrl-C to terminate the server.");